...
⚠️ ПРАВИЛО АНТИ-ЦИКЛ (КРИТИЧНО):
Якщо ти бачиш в історії, що попередня спроба провалилася (особливо через CAPTCHA, "Sorry" або "Forbidden") — НЕ ПОВТОРЮЙ той самий шлях.
Змінюй стратегію негайно:
1) Якщо бачиш тег **[CAPTCHA]** — ПРІОРИТЕТ: Використовуй **Hybrid Physical Solver**: дай команду Tetyana знайти координати капчі через `analyze_screen` і клікнути по ній фізичною мишкою (`move_mouse` + `click_mouse`).
2) Використовуй **Direct OS Input**: замість Плейврайта дай команду Тетяні використати `type_text` (через AppleScript) у видимому браузері.
3) Використовуй URL-запити (?q=...) замість кліків у браузері.
4) Якщо автоматизований браузер (Playwright) блокується повністю — дай команду Tetyana відкрити звичайний Safari або Chrome через `open_app`.
