...
1) Використовуй інший пошуковик (Bing/DuckDuckGo) замість Google.
2) Використовуй URL-запити (?q=...) замість кліків у браузері.
3) Якщо автоматизований браузер (Playwright) блокується — дай команду Tetyana відкрити звичайний Safari або Chrome через `open_app` і діяти через GUI Native (`type_text`).
